...
nd_uri_helpers_uri_to_sink (gchar *uri)
{
  ...
  switch (protocol_in_uri)
    ...
    case ND_SINK_PROTOCOL_DUMMY_CC:
      g_warning ("nd-uri-helpers: dummy CC sinks are not built into mirablast");
      sink = NULL;
      break;
    ...
    case ND_SINK_PROTOCOL_WFD_MICE:
      g_warning ("nd-uri-helpers: MICE sinks are not built into mirablast");
      sink = NULL;
      break;

    case ND_SINK_PROTOCOL_CC:
      g_warning ("nd-uri-helpers: Chromecast sinks are not built into mirablast");
      sink = NULL;
      break;
    }
